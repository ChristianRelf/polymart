...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (path === "/api/v1/getStocks") {
      ...
      let q = supabase.from("stocks_state")
        .select("ticker,name,sector,mcap,price,prev_price,hi52w,lo52w,volume,buy_volume,sell_volume,rsi,streak,bid,ask,spread_pct,vwap,session,halted,atr,beta,macd,macd_signal,macd_hist,bb_upper,bb_middle,bb_lower,bb_bw,sma20,sma50")
        .order("ticker");
      ...
      for (const s of (data || [])) {
        ...
        result[s.ticker] = {
          ...
          vwap: +s.vwap,
          session: s.session, halted: s.halted,
    ...
    if (path === "/api/v1/getSectors") {
      const [{ data: secRows }, { data: stockRows }] = await Promise.all([
        ...
        supabase.from("stocks_state").select("ticker,sector,price,prev_price,rsi,beta"),
      ]);
    ...
    if (path === "/api/v1/getSector") {
      ...
      if (!meta) return json({ error: `Unknown sector: ${sectorKey}` }, 404);
      ...
      const [{ data: secRow }, { data: stockRows }] = await Promise.all([
        ...
        supabase.from("stocks_state")
          .select("ticker,name,price,prev_price,volume,rsi,bid,ask,spread_pct,vwap,macd,macd_hist,bb_bw,session,halted")
          .in("ticker", tickers),
      ]);
      ...
      for (const s of (stockRows || [])) {
        ...
        totalChange += pct; cnt++;
        stockList.push({
          ticker: s.ticker, name: s.name, price: +s.price, change: +pct.toFixed(2),
          volume: s.volume, rsi: +s.rsi, bid: +s.bid, ask: +s.ask,
          spreadPct: +s.spread_pct, vwap: +s.vwap,
          macd: +s.macd, macdHist: +s.macd_hist, bbBw: +s.bb_bw,
          session: s.session, halted: !!s.halted,
        });
      }

      return json({
        key: sectorKey, label: meta.label, icon: meta.icon,
        avgChange: cnt > 0 ? +(totalChange / cnt).toFixed(2) : 0,
        newsStack: secRow ? +secRow.news_stack : 0,
        momentum: secRow ? +secRow.trend : 0,
        stocks: stockList, updatedAt: secRow?.updated_at ?? null,
      });
    }

    // ── GET /api/v1/getEvents ────────────────────────────────────────────────
    if (path === "/api/v1/getEvents") {
      const limitParam = Math.min(parseInt(url.searchParams.get("limit") || "10"), 40);
      const sector = url.searchParams.get("sector");
      ...
      return json((data || []).map((e: any) => ({
        id: e.id, text: e.event_text, effect: +e.effect,
        sector: e.sector ?? null, weight: e.weight,
        category: e.category ?? null, firedAt: e.fired_at,
      })));
    }
    ...
    if (path === "/api/v1/getTopMovers") {
      ...
      const { data: stocks } = await supabase.from("stocks_state")
        .select("ticker,name,sector,price,prev_price,volume,rsi,atr,beta,session,halted");

      const withChange = (stocks || []).map((s: any) => ({
        ticker: s.ticker, name: s.name, sector: s.sector,
        price: +s.price,
        change: s.prev_price > 0 ? +((s.price - s.prev_price) / s.prev_price * 100).toFixed(2) : 0,
        volume: s.volume, rsi: +s.rsi, atr: +s.atr, beta: +s.beta,
        session: s.session, halted: !!s.halted,
    ...
    if (path === "/api/v1/getLeaderboard") {
      ...
      const { data: stocks } = await supabase.from("stocks_state")
        .select("ticker,name,sector,mcap,price,prev_price,volume,rsi,ath,streak,atr,beta,spread_pct,bb_bw");

      const mapped = (stocks || []).map((s: any) => ({
        ticker: s.ticker, name: s.name, sector: s.sector, mcap: s.mcap,
        price: +s.price,
        change: s.prev_price > 0 ? +((s.price - s.prev_price) / s.prev_price * 100).toFixed(2) : 0,
        volume: s.volume, rsi: +s.rsi, ath: +s.ath, streak: s.streak,
        atr: +s.atr, beta: +s.beta, spreadPct: +s.spread_pct, bbBw: +s.bb_bw,
      }));

      const validKeys = ["change", "price", "volume", "rsi", "ath", "streak", "atr", "beta", "spreadPct", "bbBw"];
      const sortKey = validKeys.includes(by) ? by : "change";
      mapped.sort((a: any, b: any) => dir ? a[sortKey] - b[sortKey] : b[sortKey] - a[sortKey]);

      return json({ sortedBy: sortKey, direction: dir ? "asc" : "desc", count: limitParam, stocks: mapped.slice(0, limitParam) });
    }
    ...
    if (path === "/api/v1/getMacro") {
      ...
      return json({
        ...
        crashCooldown: ms.crash_cooldown, boomCooldown: ms.boom_cooldown,
        updatedAt: ms.updated_at,
      });
    }
    ...
    if (path === "/api/v1/getHistory") {
      ...
      const { data: s } = await supabase.from("stocks_state")
        .select("ticker,name,history,candles,updated_at").eq("ticker", ticker).maybeSingle();
      if (!s) return json({ error: `Ticker not found: ${ticker}` }, 404);

      const history = Array.isArray(s.history) ? s.history.slice(-limitParam) : [];
      const candles = Array.isArray(s.candles) ? s.candles : [];
      return json({ ticker: s.ticker, name: s.name, count: history.length, history, candles, updatedAt: s.updated_at });
    }

    // ── GET /api/v1/getHealth ────────────────────────────────────────────────
    if (path === "/api/v1/getHealth") {
      const { data: ms } = await supabase.from("market_state")
        .select("tick_count,updated_at,vix,market_session,fear_greed").eq("id", 1).maybeSingle();
      ...
      return json({
        ...
        vix: ms ? +ms.vix : null,
        marketSession: ms?.market_session ?? null,
        fearGreed: ms ? Math.round(ms.fear_greed) : null,
    ...
    if (path === "/api/v1/search") {
      ...
      const { data: stocks } = await supabase.from("stocks_state")
        .select("ticker,name,sector,price,prev_price,rsi,session").order("ticker");
    ...
    return json({ error: "Not found", validRoutes: [
      "/api/v1/getMarket", "/api/v1/getStocks", "/api/v1/getStock?ticker=APEX",
      "/api/v1/getSectors", "/api/v1/getSector?sector=tech",
      "/api/v1/getEvents", "/api/v1/getTopMovers", "/api/v1/getLeaderboard",
      "/api/v1/getMacro", "/api/v1/getHistory?ticker=APEX",
      "/api/v1/getHealth", "/api/v1/search?q=apex",
    ]}, 404);
