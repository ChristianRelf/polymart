import { useState, useEffect } from "react"
import { useAuth } from "@clerk/clerk-react"
import { Card, CardContent } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Loader2, Link2, ExternalLink } from "lucide-react"
import type { Route } from "@/App"

function DiscordLogo({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.013.044.03.056a19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
    </svg>
  )
}

interface Props {
  onNavigate: (r: Route) => void
}

export function DiscordLinkSection({ onNavigate }: Props) {
  const { getToken } = useAuth()
  const [linked, setLinked]         = useState<boolean | null>(null)
  const [username, setUsername]     = useState<string | null>(null)

  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        let token = await getToken()
        if (!token) {
          await new Promise(r => setTimeout(r, 300))
          token = await getToken()
        }
        if (!token || cancelled) return
        const res  = await fetch("/api/v1/account/discord/status", {
          headers: { Authorization: `Bearer ${token}` },
        })
        const json = await res.json()
        const data = json.data ?? json
        if (cancelled) return
        setLinked(!!data.linked)
        setUsername(data.discordUsername ?? null)
      } catch {
        if (!cancelled) setLinked(false)
      }
    })()
    return () => { cancelled = true }
  }, [getToken])

  return (
    <Card className="bg-card border-border">
      <CardContent className="py-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#5865F2]/10 flex items-center justify-center shrink-0">
              <DiscordLogo className="w-4 h-4 text-[#5865F2]" />
            </div>
            <div>
              <p className="text-sm font-medium leading-tight">Discord Integration</p>
              {linked === null ? (
                <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                  <Loader2 className="w-3 h-3 animate-spin" /> Checking…
                </p>
              ) : linked ? (
                <p className="text-xs text-emerald-600 mt-0.5">
                  Connected as <strong>@{username ?? "Discord User"}</strong>
                </p>
              ) : (
                <p className="text-xs text-muted-foreground mt-0.5">
                  Not linked — connect to sync your portfolio
                </p>
              )}
            </div>
          </div>

          <Button
            size="sm"
            variant={linked ? "outline" : "default"}
            onClick={() => onNavigate("account-link")}
            className="shrink-0 gap-1.5"
          >
            {linked ? (
              <><ExternalLink className="w-3.5 h-3.5" /> Manage</>
            ) : (
              <><Link2 className="w-3.5 h-3.5" /> Link Discord</>
            )}
          </Button>
        </div>
      </CardContent>
    </Card>
  )
}
